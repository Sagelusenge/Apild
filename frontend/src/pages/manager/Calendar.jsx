...
const initialForm = () => ({ title: '', project_id: '', start_time: '09:00', end_time: '10:00', location: '', meeting_url: '', reminder_minutes: '30', participant_ids: [] });
...
export default function Calendar() {
  ...
  const load = useCallback(async () => {
    ...
    const [eventResult, projectResult, staffResult] = await Promise.allSettled([
      eventsApi.list({ page: 1, limit: 100, sortBy: 'starts_at', sortOrder: 'asc' }),
  ...
  const firstCurrentMonth = new Date(new Date().getFullYear(), new Date().getMonth(), 1);
  const previousMonthDisabled = month <= firstCurrentMonth;

  const selectDate = (date) => {
    if (isBeforeToday(date, today)) return;
  ...
  const createEvent = async (event) => {
    ...
    try {
      ...
      notify(form.participant_ids.length ? 'Réunion créée : invitations et rappels mis en file d’attente.' : 'Réunion ajoutée au calendrier.', 'success');
  ...
  return <section className="calendar-page">
    <header className="resource-title">
      <div><span className="eyebrow"><CalendarDays size={14} /> Planification</span><h1>Calendrier</h1><p>Planifiez une réunion, choisissez les participants et le délai de rappel.</p></div>
    ...
    <div className="calendar-layout">
      <section className="card calendar-board" aria-label="Calendrier mensuel">
        <header className="calendar-board__header"><button className="icon-button" type="button" onClick={() => setMonth((current) => new Date(current.getFullYear(), current.getMonth() - 1, 1))} aria-label="Mois précédent" disabled={previousMonthDisabled}><ChevronLeft /></button><h2>{monthLabel(month)}</h2><button className="icon-button" type="button" onClick={() => setMonth((current) => new Date(current.getFullYear(), current.getMonth() + 1, 1))} aria-label="Mois suivant"><ChevronRight /></button></header>
    ...
    <Modal open={isComposerOpen} title="Planifier une réunion" onClose={() => !saving && setComposerOpen(false)}><form className="form-grid calendar-composer" onSubmit={createEvent}>
      ...
      <div className="field"><label htmlFor="calendar-reminder">Rappel par e-mail (minutes avant)</label><input id="calendar-reminder" type="number" min="0" max="43200" value={form.reminder_minutes} onChange={(event) => setForm((current) => ({ ...current, reminder_minutes: event.target.value }))} required /><small>Indiquez 0 pour ne pas envoyer de rappel.</small></div>
      <div className="field"><label htmlFor="calendar-location">Lieu <small>(facultatif)</small></label><input id="calendar-location" value={form.location} onChange={(event) => setForm((current) => ({ ...current, location: event.target.value }))} placeholder="Ex. Salle de réunion APILD" /></div>
      <div className="field"><label htmlFor="calendar-link">Lien de réunion <small>(facultatif)</small></label><input id="calendar-link" type="url" value={form.meeting_url} onChange={(event) => setForm((current) => ({ ...current, meeting_url: event.target.value }))} placeholder="https://…" /></div>
      <fieldset className="field field--full calendar-participants"><legend><Users size={16} /> Personnel invité <small>(facultatif)</small></legend>{staff.length ? <div className="calendar-participants__list">{staff.map((person) => <label key={person.id} className="calendar-participant"><input type="checkbox" checked={form.participant_ids.includes(person.id)} onChange={() => toggleParticipant(person.id)} /><span><strong>{person.first_name} {person.last_name}</strong><small>{person.job_title || person.email}</small></span></label>)}</div> : <p>Aucun membre du personnel actif n’est disponible.</p>}<small>Chaque participant recevra une invitation avec le programme et le rappel choisi.</small></fieldset>
