...
const columnsByResource = {
  ...
  documents: ['title', 'original_name', 'document_type', 'share_scope', 'version_number', 'created_at']
};
...
const display = (value) => {
  ...
  const translated = { private: 'Privé', team: 'Toute l’équipe', selected: 'Personnes choisies' }[value] || value;
  return String(translated).length > 70 ? `${String(translated).slice(0, 70)}…` : String(translated);
};
...
function validateValues(form, values, mode) {
  ...
  for (const field of form.fields) {
    if (field.visibleWhen && values[field.visibleWhen.field] !== field.visibleWhen.value) continue;
    if (field.special || field.autoCalculated || field.autoGenerated) continue;
    const value = values[field.name];
    const required = field.required || (mode === 'create' && field.requiredOnCreate) || (field.requiredWhen && values[field.requiredWhen.field] === field.requiredWhen.value);
...
function ResourceForm({ form, mode, record, lookups, lookupsError, onCancel, onSave, canManageUserAccounts = false }) {
  ...
  return <form className={`form-grid${form.layout ? ` resource-form--${form.layout}` : ''}`} noValidate onSubmit={submit}>
    ...
    {form.fields.filter((field) => {
      ...
      const isVisible = !field.visibleWhen || values[field.visibleWhen.field] === field.visibleWhen.value;
      return isVisible && (!isUserAdministrationField || canManageUserAccounts);
    }).map((field) => {
...
export default function ResourcePage({ title, description, resource, canCreate = true, canEdit = true, canDelete = false, canManageAccountStatus = false, printable = false, secondaryAction = null }) {
  ...
  const save = async (values) => {
    ...
    if (editor.mode === 'create' && form.uploadEndpoint) {
      ...
      const data = new FormData();
      Object.entries(payload).forEach(([key, value]) => data.append(key, Array.isArray(value) ? JSON.stringify(value) : String(value)));
      data.append('file', file);
