...
export default function ProjectCard({ project, image, delay = 0 }) {
  ...
  return <Reveal as="article" className="project-card card" delay={delay}><div className="project-image"><img src={image} alt="" /><Badge tone="success">{localizedProject.status === 'completed' ? text.cards.completed : text.cards.ongoing}</Badge></div>
    <div className="project-body"><small>{localizedProject.reference}</small><h3>{localizedProject.name}</h3><p>{localizedProject.description || text.cards.projectFallback}</p>
      ...
      <div className="progress"><span><i style={{ width: `${localizedProject.progress_percent || 0}%` }} /></span><b>{localizedProject.progress_percent || 0}%</b></div>
