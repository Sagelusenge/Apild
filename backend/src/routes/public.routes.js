...
router.get('/interventions', asyncHandler(async (_request, response) => success(response, await db.query(`SELECT i.id,i.reference,i.title,i.description,i.intervention_date,i.province,i.territory,i.locality,i.beneficiaries_men,i.beneficiaries_women,i.beneficiaries_children,d.name AS domain_name FROM interventions i JOIN intervention_domains d ON d.id=i.domain_id WHERE i.deleted_at IS NULL AND i.status='completed' ORDER BY i.intervention_date DESC`))));
...
router.get('/impact', asyncHandler(async (_request, response) => {
  const rows = await db.query(`SELECT (SELECT COUNT(*) FROM projects WHERE deleted_at IS NULL AND status IN ('active','completed')) AS projects,(SELECT COUNT(*) FROM interventions WHERE deleted_at IS NULL AND status='completed') AS interventions,(SELECT COALESCE(SUM(beneficiaries_men+beneficiaries_women+beneficiaries_children),0) FROM interventions WHERE deleted_at IS NULL AND status='completed') AS beneficiaries,(SELECT COUNT(*) FROM partners WHERE deleted_at IS NULL AND status='active') AS partners`);
  return success(response, rows[0]);
}));
